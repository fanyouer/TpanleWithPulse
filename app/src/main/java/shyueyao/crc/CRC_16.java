...
static  final  String HEXES="0123456789ABCDEF";
 ...
 /*
 public  int   CRC_value(byte[] puchMsg,int usDataLen)
 {
	 int   uIndex;
	 int   value1;
	   for(int i=0;i<usDataLen;i++)
	     {
		    uIndex=(uchCRCHi^puchMsg[i])&0xff;
	         ...
	         value1=( ( ((int)uchCRCHi) <<8  |  (( (int)uchCRCLo) & 0xff) ) ) & 0xffff;
	         return value1    ;
    }
 ...
 private  static  byte[] HexString2Buf(String src )
 {
	 ...
	 for(int i=0;i<len;i+=2)
	       {
	    	 ret[i/2]=unitBytes(tmp[i], tmp[i+1]);		
	       }
	     return ret;
 }
 ...
 public   static  byte[]getSenddataBuf(byte[]  send)
 {
	 CRC_16   crc16 = new CRC_16();  
	   crc16.update(send, send.length-2);  
	     int ri = crc16.getValue();  
	     send[send.length-1]=(byte) (0xff & ri); 

	     send[send.length-2]=(byte) ((0xff00 & ri) >> 8); 
	  return send; 
 }
